...
export class PopularProductsManagerComponent implements OnInit {
  ...
  loadData() {
    this.db.collection('/popular').valueChanges().subscribe((popular_items: any[]) => {
      ...
      this.flatPopularProducts = popular_items_sorted;
  ...
  drop(event: CdkDragDrop<string[]>) {
    if (event.currentIndex !== event.previousIndex) {
      moveItemInArray(this.newOrder, event.previousIndex, event.currentIndex);
    }
  }

  openChangePopularProductOrder() {
    this.newOrder = [];
    this.flatPopularProducts.forEach(element => {
      this.newOrder.push(element);
    });
    console.log(this.newOrder);
    this.openModal('changePopularProductOrderModal');
  }

  submitChangePopularProductOrder() {
    const batch = this.db.firestore.batch();
    for (let i = 0; i < this.newOrder.length; i++) {
      batch.update(this.db.collection('popular').doc(this.newOrder[i].db_name).ref, {
        display_order: i + 1
      });
    }
    batch.commit();
    this.closeModal('changePopularProductOrderModal');
  }
}
